...
const secLabel = { fontSize: '10px', fontWeight: 700, letterSpacing: '.1em', color: '#768390' };
...
export function PulseDock() {
    ...
    return (
        ...
        <aside style={{ position: 'fixed', top: 'calc(64px + var(--app-top-offset, 0px))', right: '12px', bottom: '12px', width: '300px', zIndex: 45, border: '1px solid rgba(255,255,255,.07)', borderRadius: '16px', background: '#14161b', display: 'flex', flexDirection: 'column', fontFamily: 'var(--font-sans)', boxShadow: '0 16px 48px rgba(0,0,0,.5)', overflow: 'hidden', animation: 'pulseSlideIn .22s ease' }}>
            {/* header */}
            <div style={{ height: '54px', flexShrink: 0, boxSizing: 'border-box', display: 'flex', alignItems: 'center', gap: '9px', padding: '0 16px', borderBottom: '1px solid #30363d' }}>
                ...
                <div className="nav" onClick={togglePulse} title="Hide Pulse" style={{ width: '28px', height: '28px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#768390', cursor: 'pointer' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
                ...
                {runs.length > 0 && (
                    <>
                        {runs.map((r) => (
                            <div key={r.id} className="nav" onClick={() => navigate(`/forge/runs/${r.id}`)} style={{ borderRadius: '9px', background: '#1c2128', border: '1px solid #30363d', padding: '10px', marginBottom: '8px', cursor: 'pointer' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '6px' }}>
                                    ...
                                    {r.cost_usd > 0 && <span style={{ fontSize: '10px', color: '#80cbc4' }}>${r.cost_usd.toFixed(2)}</span>}
                                </div>
                                <div style={{ fontSize: '11px', color: '#b1bac4', marginBottom: '7px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                ...
                                <div style={{ height: '3px', borderRadius: '999px', background: 'rgba(56,189,248,.1)', overflow: 'hidden', position: 'relative' }}>
                                    <span style={{ position: 'absolute', top: 0, height: '100%', width: '35%', borderRadius: '999px', background: 'linear-gradient(90deg,rgba(56,189,248,0),#38bdf8,#7dd3fc)', animation: 'barwork 1.5s ease-in-out infinite' }} />
                                </div>
                            </div>
                        ))}
                    </>
                ...
                {waiting.length > 0 && (
                    <>
                        {waiting.map((r) => (
                            <div key={r.id} className="nav" onClick={() => navigate(`/forge/runs/${r.id}`)} style={{ borderRadius: '9px', background: '#1c2128', border: '1px solid rgba(255,152,0,.3)', padding: '10px', marginBottom: '8px', cursor: 'pointer' }}>
                                ...
                                <div style={{ fontSize: '11px', color: '#b1bac4', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {(r.task_key || r.task_id) && <span style={{ fontFamily: 'ui-monospace,monospace', color: '#7c8db5' }}>{r.task_key || r.task_id}</span>} {r.task_title || 'Needs your input'}
                                </div>
                            </div>
                        ))}
                    </>
                        ...
                        {notifs.map((n) => (
                            <div key={n.id} className="nav" onClick={() => n.link && navigate(n.link)} style={{ borderRadius: '9px', padding: '9px', marginBottom: '2px', cursor: n.link ? 'pointer' : 'default' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '3px' }}>
                                    <span style={{ width: '18px', height: '18px', borderRadius: '50%', background: 'rgba(128,203,196,.14)', color: '#80cbc4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: 700, flexShrink: 0 }}>{initials(n.type)}</span>
                                    ...
                                    <span style={{ fontSize: '10px', color: '#768390', flexShrink: 0 }}>{timeAgo(n.created_at)}</span>
                                </div>
                                {n.type && <div style={{ fontSize: '11px', color: '#768390', paddingLeft: '25px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.type}</div>}
                            </div>
                        ))}
                    </>
                )}
...
function EmptyRow({ children }) {
    return (
        <div style={{ fontSize: '11.5px', color: '#768390', padding: '6px 2px 2px', lineHeight: 1.5 }}>
            {children}
        </div>
    );
}
