...
const DOT_RING = ['var(--text-muted)', '#58a6ff', '#ff9800', 'var(--pulse-blue)', '#2ecc71'];
...
const Arrow = ({ s = 20, stroke = 'var(--border-strong)' }) => (
...
function GateBadge({ n }) {
    const locked = n > 0;
    const color = locked ? 'var(--brand-teal)' : 'var(--border-strong)';
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', flexShrink: 0, width: '48px', paddingTop: '14px' }}>
            <Arrow s={18} stroke={locked ? 'var(--brand-lavender)' : 'var(--border-strong)'} />
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', fontSize: '9px', color, background: locked ? 'rgba(128,203,196,.1)' : 'transparent', border: `1px solid ${locked ? 'rgba(128,203,196,.25)' : 'var(--border-default)'}`, borderRadius: '5px', padding: '2px 5px' }}>
...
function ColumnTile({ name, tasks, dotColor, selected, onSelect }) {
    ...
    if (selected) {
        return (
            <div onClick={() => onSelect(name)} style={{ position: 'relative', background: '#191620', border: '2px solid var(--brand-lavender)', borderRadius: '11px', padding: '9px', width: '124px', flexShrink: 0, cursor: 'pointer', boxShadow: '0 0 0 4px rgba(201,184,255,.14),0 0 28px rgba(201,184,255,.22)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '6px', marginBottom: '8px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-primary)' }}>{name}</span>
                    <Gear />
                </div>
                <div style={{ background: 'var(--surface-sunken)', border: '1px solid var(--pulse-blue)', borderRadius: '6px', padding: '6px', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 0 12px rgba(56,189,248,.18)' }}>
                    <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: 'var(--pulse-blue)', animation: 'pulsedot 2800ms ease-in-out infinite' }} />
                    <span style={{ fontFamily: 'ui-monospace,SFMono-Regular,Menlo,monospace', fontSize: '8px', color: 'var(--pulse-blue-bright)' }}>{chipKey || `${tasks.length} tasks`}</span>
                </div>
            </div>
        );
    }
    return (
        <div onClick={() => onSelect(name)} style={{ background: 'var(--surface-raised)', border: '1px solid var(--border-default)', borderRadius: '10px', padding: '10px', width: '116px', flexShrink: 0, opacity: 0.5, cursor: 'pointer', transition: 'opacity .12s' }}
            ...
            <div style={{ background: 'var(--surface-sunken)', border: '1px solid var(--border-default)', borderRadius: '6px', padding: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: dotColor }} />
                <span style={{ fontSize: '9px', color: 'var(--text-muted)' }}>{tasks.length}</span>
            </div>
        </div>
    );
}
        ...
        <div style={{ position: 'relative', minHeight: '100%', backgroundColor: '#0a0d14', backgroundImage: 'radial-gradient(#1a2333 1px, transparent 1px)', backgroundSize: '22px 22px', padding: '30px 26px' }}>
            <div style={{ position: 'relative', background: 'rgba(20,25,33,.55)', border: '1px solid #2a313c', borderRadius: '14px', padding: '20px 18px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center', marginBottom: '16px' }}>
                    <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--brand-lavender)' }} />
                    <span style={{ fontSize: '10px', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--text-section)', fontWeight: 600 }}>The board · every column has a process &amp; a gate</span>
                </div>
                ...
                <div style={{ textAlign: 'center', marginTop: '4px' }}>
                    <span style={{ fontSize: '9.5px', color: 'var(--text-muted)' }}>Selecting the <span style={{ color: 'var(--brand-lavender)' }}>{selected}</span> column — its process, gate &amp; prompt are on the right</span>
                </div>
            </div>
        </div>
    );
