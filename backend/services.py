...
_KICKOFF_TASK_DESCRIPTION = (
    "You are kicking off a new project. Read the project description and "
    "any attachments the user uploaded.\n\n"
    "**Reading the attachments (project-level, not task-level):**\n\n"
    "1. Call `list_project_attachments(AGENTIRA_PROJECT_ID)` — returns "
    "metadata for every file dropped on the project. Text files "
    "(markdown briefs, READMEs, JSON specs, YAML configs) come back "
    "with content already inlined under `inline_text` — no extra fetch.\n"
    "2. For binary files (zips, images, PDFs), each item carries a "
    "`download_url` and `api_key_env: \"AGENTIRA_API_KEY\"`. Fetch with:\n\n"
    "    curl -H \"Authorization: Bearer $AGENTIRA_API_KEY\" "
    "http://backend:8000<download_url> -o /tmp/file\n\n"
    "   For zips, `unzip` and read what you need.\n"
    "3. Do NOT use the old `list_attachments(task_id)` / "
    "`download_attachment` tools — those are task-scoped and won't see "
    "the project-level files.\n\n"
    "**Once you have the brief:**\n\n"
    "1. **Pick the tools/tech stack** that fit the product, with a "
    "brief justification for each choice.\n"
    "2. **Write a one-page plan** covering architecture, milestones, "
    "and risks. Register it as a real artifact via "
    "`register_run_artifact(kind='report', label='Project plan')` so "
    "it shows up on the Run page — don't just emit it as chat text.\n"
    "3. **Break the work into 3–8 concrete tasks** using `create_task`. "
    "Each gets a clear title, description, and DoD items.\n\n"
    ...
    "ambiguous requirement), call `finish_run(outcome='needs_input')` "
    "with a specific question — a follow-up comment on this task will "
    "resume you."
)
